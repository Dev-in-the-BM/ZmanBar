...
class SunTimesCalculator extends AstronomicalCalculator_1.AstronomicalCalculator {
     ...
     */
    static getTimeUTC(date, geoLocation, zenith, isSunrise) {
        const dayOfYear = date.ordinal;
        const sunMeanAnomaly = SunTimesCalculator.getMeanAnomaly(dayOfYear, geoLocation.getLongitude(), isSunrise);
        ...
        const cosLocalHourAngle = SunTimesCalculator.getCosLocalHourAngle(sunTrueLong, geoLocation.getLatitude(), zenith);
        let localHourAngle;
        if (isSunrise) {
        ...
        const localMeanTime = SunTimesCalculator.getLocalMeanTime(localHour, sunRightAscensionHours, SunTimesCalculator.getApproxTimeDays(dayOfYear, SunTimesCalculator.getHoursFromMeridian(geoLocation.getLatitude()), isSunrise));
    ...
    getUTCNoon(date, geoLocation) {
        const sunrise = this.getUTCSunrise(date, geoLocation, 90, false);
        const sunset = this.getUTCSunset(date, geoLocation, 90, false);
